...
```
<div id="top">
__Name: Umais Siddiqui__

__Class: Data 606 Fall 2020__

<ul>
<li><a href="#one">Click here to see Answer to Exercise 1</a></li>

<li><a href="#two">Click here to see Answer to Exercise 2</a></li>
<li><a href="#three">Click here to see Answer to Exercise 3</a></li>
<li><a href="#onyourown">Click here to Go to "On Your Own" section</a></li>
</ul>
<div>
...
arbuthnot$boys
```
<a href="#top" class="lnk" ><u><b>Back to Top</b></u></a>
...
<div style="color:black;background-color:yellow;font-weight:bold;">
<span style="color: red;font-weight:bold">Answer:</span>
<p>Yes there is an apparent trend in the number of girls baptized each year. Inititally the numbers declined a bit then increased and then there was a steep decline starting 1640 which lasted until 1660 then the numbers started to climb up again until 1700s.
</p>
</div>
</div>
...
```{r plot-proprtion-boys, eval=TRUE }
ProportionOfBoys<-arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls)
plot(arbuthnot$year,ProportionOfBoys,"l")
```
...
-   What years are included in this data set? What are the dimensions of the 
    ...
    <span style="color:red">The years included in the data set are</span>
    
    
    
    ```{r Years-Included , eval=TRUE}
      present$year
    ```
    ...
    ```{r Present-Dimensions, eval=TRUE}
    dim(present)
    ```
    ...
    ```{r present-column-name, eval=TRUE}
    names(present)
    ```
...
__Total Birth Arbuthnot Years__
...
<span style="color:red;font-weight:bold">As compared to the Arbuthnot's counts Present counts are significantly higher as they are in the millions per year however, in arbuthnot's data set they were in thousands.</span>
...
-   Make a plot that displays the boy-to-girl ratio for every year in the data 
    ...
    <span style="color: red;font-weight:bold">Answer</span>
      ...
      __Below is the ratio between boy-to-girl__
    
    
    
    ```{r Present-Boy-To-Girl-Ratio, eval=TRUE}
    
    
       boygirlratio <- present$boys/present$girls
    
      plot(present$year,boygirlratio,"l")
    ```


<span style="color:red">Yes as we can see from the graph above the observation made by Arbuthnot about boys being born in greater proportion also holds true in the U.S</span>
...
-   In what year did we see the most total number of births in the U.S.? You can
    ...
    <span style="color:red">The year that the max total number of births occured can be found out by using the formula below. </span>
    
    
    ```{r most_births_us,eval=TRUE}
...
```
    ...
    <span style="color:red">We can also see that in a plot below<span>

    ```{r plot_for_total_births_by_year, eval=TRUE}


   ggplot(data = present) + 
  geom_point(mapping = aes(x = present$year, y = present$boys+present$girls))
```
