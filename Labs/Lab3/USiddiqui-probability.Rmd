...
Streak length of 1 means that Kobe hit 1 basket until he missed a basket.A streak ends at a miss of the basket so f Kobe had a Streak length of 1 then he misses another shot then that would be a streak of 0. He will continue getting streaks of Zero if he keeps missing.
...
In order to understand Kobe's Streaks lets look at the summary of his Streaks and also lets draw a box plot
...
Based on the summary and by looking at the boxplot we can easily say that the distribution is right skewed as there is a natural boundry at 0 and most of his shots are 0 streaks making his typical streaks the length of 0 based on the median and if we go with the mean his typical streak length is 0.732. Since in a right skewed distribution we should prefer using the median to describe the typical value it means 0 is the typical value. The longest streak that kobe had and we can see that from the summary above as well is 4
...
```{r}
result<-table(sim_unfair_coin)
result
```

<div>
<p style="padding:10px;border:1px solid silver;border-radius:5px;width:100%;height:40px;">
As seen in the table above the number of times heads came up for my simulation is `r result['heads'] `
</p>
</div>
...
<div>
<p style="padding:10px;border:1px solid silver;border-radius:5px;width:100%;height:40px;">
In order to make the shooting percentage 45% we will have to pass an extra parameter to the `sample` function
this parameter will be `prob` parameter and will essentially specify the probability of H or M where probabilty of H will be set to .45. see below.
...
I will be using the  `calc_streak` function to calculate the streak lengths of the `sim_basket` and store the results in `sim_streak` variable see below.


```{r simualted-player-streak}
...
<b>1.   Describe the distribution of streak lengths. What is the typical streak 
    length for this simulated independent shooter with a 45% shooting percentage?
    How long is the player's longest streak of baskets in 133 shots?</b>
...
summary<-summary(sim_streak)
summary
```


<div>
<p style="padding:10px;border:1px solid silver;border-radius:5px;width:100%;height:140px;">
The distribution is Right Skewed similar to how we saw Kobe's Streak distribution therefore median would be the best way to describe a typical streak. The typical shooting streak is `r median(sim_streak) ` as we can see from the median value in the summary table . Also the Max. value in the table tells us the longest streak which is `r summary['Max.'] `. 
</p>
</div>



<b>2.   If you were to run the simulation of the independent shooter a second time, 
    ...
    different? Explain your reasoning.</b>
    
### Answer 
 
<div>
<p style="padding:10px;border:1px solid silver;border-radius:5px;width:100%;height:140px;">   
I would expect it to be somewhat similar not exactly the same as the simulation may generate different results every time and results can vary but generally speaking it will be the similar in the sense that the typical streak lengths will be between 0 and 1 and the distribution will be Skewed to the right .
</p>
</div>

<b>3.   <b>How does Kobe Bryant's distribution of streak lengths compare to the 
...
<div>
<p style="padding:10px;border:1px solid silver;border-radius:5px;width:100%;height:140px;">
the distribution of Kobe's streak length vs simulated shooter's streak length are very similar since both are right skewed with majority of the streak lengths being 0 or 1. I believe the Hot hand model describes Kobe's shooting patterns accurately as the simulated shooter has the same percentage of shooting as Kobe and the results of the streak were very similar.
</p>
</div>
