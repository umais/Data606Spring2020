...
Total of `r ncol(cdc)` variables


A total of `r nrow(cdc)` cases are in this data.
...
**We will do the summary and IQR for Height First**
...
height_summary["IQR"]<-height_summary["3rd Qu."]- height_summary["1st Qu."]
...
**Now we will do the summary and IQR for age**    
...
age_summary["IQR"]<-age_summary["3rd Qu."]- age_summary["1st Qu."]

age_summary

```
    
Relative Frequency distribution for the `gender` is 

```{r}
table(cdc$gender)/nrow(cdc)
```

Similarly we can do the relative frequency distribution for the `exerany` variable

```{r}
table(cdc$exerany)/nrow(cdc)
```

To answer the question about what proportion of population reports being in excellent health we will first do a summary of general health.

```{r}
table(cdc$genhlth)
```

Now to see the proportion of population reporting in excellent health we can just do the following by displaying the relative frequency distribution.

```{r}
rel_freq<-table(cdc$genhlth)/nrow(cdc)

rel_freq
```

So we can see from the above distribution that the proportion reporting to be in excellent health are `r rel_freq["excellent"] `
...
The Mosaic Plot tells us that there are more male who have smoked atleast 100 cigarettes than females and there are more females who smoked less than 100 cigarettes than male. It reveals that men tend to smoke more than women.Although we can also see the same results in the table command mosaic plot depicts this fact visually to get a better sense of the difference. 
...
The box plot shows that on average people with lower BMI have better health because the higher the median BMI gets the worst the health gets also the first and third quartile are also higher in the people with poorer health.People with higher than 35 bmi are pretty much spread across in equal numbers in the different health categories.
...
So I used the smoke 100 variable to compare with the BMI and it suggests that the median for both smoking atleast 100 cigarettes and not smoking 100 cigarretes pretty much have the same median bmi. However, there is one outlier in the smoke 100 that has a very high BMI and also two outliers tat have a very low BMI and smoke atleast 100 cigarettes. The first quartile and the 3rd quartile are pretty much the same for both.
...
The relationship shows that most of the people want there desired weight to be lower than there actual weight and it shows a very interesting relationship . People less than a 100 pounds want to increase there weight almost all people who are 200 and above want there weight to be lower than 200.
...
wdiff is numerical discrete data. If wdiff is 0 it means the person is satisfied with their current weight. If wdiff is positive it means that the person wants to lower their weight and if it is negative then the person wants to increase their weight.
...
lowerrange<-weight_mean-weight_sd

upperrange<-weight_mean-weight_sd

people_within_one_sd<- subset(cdc,cdc$weight>=lowerrange & cdc$weight>=upperrange)

proportion_within_one_sd<-nrow(people_within_one_sd)/nrow(cdc)
...
the above shows us that 86% of the people are within one sd of the mean. By looking at the histogram below we can see that it makes sense.
